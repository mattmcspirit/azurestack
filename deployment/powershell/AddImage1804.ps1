...
if (($progressCheck -eq "Incomplete") -or ($progressCheck -eq "Failed")) {
    try {
        ...
        elseif ($image -eq "UbuntuServer") {
            $sku = "18.04-LTS"
            $onlinePackage = "*Canonical.UbuntuServer1804LTS*"
            $offlinePackage = "Canonical.UbuntuServer1804LTS-ARM.1.0.0"
            $publisher = "Canonical"
            $offer = "UbuntuServer"
        ...
        if ($registerASDK -and ($deploymentMode -eq "Online")) {
            ...
            if ($image -eq "UbuntuServer") {
                # Get download information for Ubuntu Server 18.04 LTS VHD file
                ...
                if ($validDownloadPathVHD -eq $true) {
                    ...
                    if ($image -eq "UbuntuServer") {
                        ...
                        if ($validDownloadPathZIP -eq $true) {
                            ...
                            $serverVHD = Get-ChildItem -Path "$csvImagePath\Images\$image\" -Filter *amd64.vhd | Rename-Item -NewName "$blobName" -PassThru -Force -ErrorAction Stop
                        }
                        else {
                            ...
                            if ($registerASDK -and ($deploymentMode -eq "Online")) {
                                ...
                                $ubuntuURI = "https://cloud-images.ubuntu.com/releases/18.04/release-$ubuntuBuild/ubuntu-18.04-server-cloudimg-amd64.vhd.zip"

                            }
                            elseif (!$registerASDK -and ($deploymentMode -eq "Online")) {
                                $ubuntuURI = "https://cloud-images.ubuntu.com/releases/bionic/release/ubuntu-18.04-server-cloudimg-amd64.vhd.zip"
                            }
